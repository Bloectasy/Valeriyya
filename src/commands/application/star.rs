...
pub async fn star(
    ...
    if let Some(starboard_channel) = starboard_channel {
        let webhook = serenity::ChannelId::new(
            starboard_channel
                .parse::<u64>()
                .expect("Can't parse the starboard_channel id"),
        )
        .to_guild_channel(ctx.http(), Some(serenity::GuildId::new(guild_id)))
        .await?
        .create_webhook(
            ctx.http(),
            serenity::CreateWebhook::new(msg.author.display_name()).avatar(
                ...
                .encode()
                .await
                .expect("Failed to make the avatar image for the webhook!"),
            ),
        )
        .await
        .unwrap();
        ...
        for attachment in msg.attachments.iter() {
            ...
            match serenity::CreateAttachment::url(ctx.http(), url.as_str(), filename).await {
        ...
        webhook
            .delete(ctx.http(), Some("Starred a Message"))
            .await?;
    } else {
        ctx.reply(
            "There is no starboard channel set!\nUse `settings channels starboard <channel>`",
        )
        .await?;
    };
